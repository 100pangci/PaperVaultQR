...
- エンコード前に QR レイアウト設定を調整可能：
  ...
  - 1 ページの列数
  - ページ余白（cm）
- **デフォルトに戻す**で、上記パラメータをコード内蔵の既定値に一括復元
...
- GUI モードではエンコード前にこれらを変更でき、**デフォルトに戻す**で上記既定値へ復元できます
...
- `313 KB` の内容をこのソフトでエンコードし、`642` 個の QR コードを生成。
- 印刷後に順番にスキャンしたところ、`2` 個だけ読み取りに失敗。
- その `2` 枚を単独でスクリーンショット化して同じフォルダに置いたところ、最終的に正常に復元できた。
